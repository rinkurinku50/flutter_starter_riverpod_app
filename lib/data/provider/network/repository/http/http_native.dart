...
import 'core/http_contract.dart';

@Singleton(as: Network, env: <String>[Env.androidStaging, Env.androidProduction, Env.iosStaging, Env.iosProduction])
class HttpNetworkNative implements Network {
  final HttpNetwork _httpNetwork;

  HttpNetworkNative(this._httpNetwork);
  ...
  Future<ApiResult> callApi({required NetworkMethod method}) async {
    ...
    try {
      ...
      CustomPrint.printException("http exception", exception.toString(), stacktrace.toString());
      return ApiResult.failure(networkException: NetworkException.httpException(exception: exception));
    } on SocketException catch (exception, stacktrace) {
      CustomPrint.printException("socket exception", exception.toString(), stacktrace.toString());
      return ApiResult.failure(networkException: NetworkException.socketException(exception: exception));
